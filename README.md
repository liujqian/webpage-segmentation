...
given in the official yolox repository. You can use the same exp file for inference. To evaluate the inference results, use the batch_process_inference_results.py file. 
...
but the configuration file used for inference is placed in the htc/customized-configs folder (the only python file in that folder). The second 
configuration file is basically the same configuration file, but it is flattened and contains some other minor changes that are not relevant for the inference process. 
